...
async function startServer() {
    ...
    await downloader_api.backfillNotPublicDownloadErrorTypes().catch(error => logger.warn(`Not-public backfill failed: ${error}`));
...
// Transitional global API key (fork feature): when ytdl_use_api_key is enabled, a
// request may present the configured key as ?apiKey=... to pass this gate. Per-user
// Bearer tokens (authentication/optional-jwt.js) remain the primary mechanism; this
// accepts the fork's single-key clients until they migrate.
...
function globalApiKeyMatches(presented_key) {
    return !!presented_key
        && !!config_api.getConfigItem('ytdl_use_api_key')
        && presented_key === config_api.getConfigItem('ytdl_api_key');
}

app.use(function(req, res, next) {
    if (!req.path.includes('/api/')) {
        next();
    } else if (config_api.getConfigItem('ytdl_multi_user_mode')) {
        // Multi-user deployments authenticate per user (Bearer tokens / sessions +
        // route guards); the global key only exists there as an optional admin
        // shortcut, resolved in optional-jwt.js.
        next();
        ...
        next();
    } else if (globalApiKeyMatches(req.query.apiKey)) {
        next();
...
// Transitional (fork feature): rotate the shared global API key. The per-user
// Bearer-token equivalents are /api/generateAPIToken and friends above.
