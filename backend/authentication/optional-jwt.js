...
/*************************************************
 * Transitional global API key (fork feature).
 *
 * A single shared key configured under
 * YtdlMaterial.API.API_key. It carries no identity of
 * its own, so in multi-user mode it resolves to the
 * admin account -- full machine access is what the
 * shared key has always meant. Single-user mode never
 * reaches here: the gate in app.js already admitted
 * the request and there are no accounts to resolve.
 ************************************************/
async function logInWithGlobalApiKey(req) {
    if (!config_api.getConfigItem('ytdl_multi_user_mode')) return false;
    if (!req.query || typeof req.query.apiKey !== 'string') return false;
    if (!config_api.getConfigItem('ytdl_use_api_key')) return false;
    if (req.query.apiKey !== config_api.getConfigItem('ytdl_api_key')) return false;

    const admin_user = await db_api.getRecord('users', {uid: 'admin', role: 'admin'});
    if (!admin_user) return false;

    await new Promise((resolve, reject) => req.logIn(admin_user, {session: false}, err => err ? reject(err) : resolve()));
    req.authenticated_with_api_key = true;
    return true;
}

exports.logInWithGlobalApiKey = logInWithGlobalApiKey;
...
// either credential. The transitional global key gets the same refusal: a shared
// secret must not be able to mint a per-user credential.
exports.requireJwtForTokenManagement = function (req, res, next) {
    if (config_api.getConfigItem('ytdl_multi_user_mode') && (req.authenticated_with_api_token || req.authenticated_with_api_key)) {
        res.sendStatus(403);
        return;
    }
    next();
...
exports.resolveJwtIfPresent = async function (req, res, next) {
    ...
    if (await logInWithGlobalApiKey(req)) return next();
...
exports.optionalJwt = async function (req, res, next) {
    ...
    if (multiUserMode && ((req.body && req.body.uuid) || (req.query && req.query.uuid)) && SHARED_LINK_PATHS.has(req.path)) {
        ...
        // Transitional: the fork's shared global key resolves to the admin account.
        if (await logInWithGlobalApiKey(req)) return next();
        if (!req.query.jwt) {
            res.sendStatus(401);
            return;
        ...
        await logInWithApiToken(req);
        return next();
    } else if (multiUserMode && req.query && req.query.apiKey) {
        await logInWithGlobalApiKey(req);
        return next();
