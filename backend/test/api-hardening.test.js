...
describe('The shared bootstrap secret', function() {
    ...
    it('is not required by the API gate', function() {
        // Fork divergence: the transitional global API key gate exists (see the
        // globalApiKeyMatches helper), but it must stay opt-in and must never
        // apply to multi-user deployments, where identity comes from
        // optionalJwt and the route guards instead. Closing the socket for a
        // keyless request is only acceptable behind those two conditions.
        const app_source = fs.readFileSync(path.join(__dirname, '..', 'app.js'), 'utf8');

        assert(app_source.includes('ytdl_multi_user_mode'), 'the gate lost its multi-user bypass');
        const gate_start = app_source.indexOf('function globalApiKeyMatches');
        assert(gate_start !== -1, 'the transitional gate helper is gone');
        const gate_source = app_source.slice(gate_start, app_source.indexOf('app.use(compression())'));
        assert(gate_source.includes('ytdl_use_api_key'),
            'the gate must check the opt-in toggle before comparing keys');
        assert(gate_source.includes('req.socket.end()'),
            'the gate should still hard-close callers that present a wrong key');
    });

    it('keeps the transitional global API key roped off from token minting', function() {
        // The configured shared key authenticates as admin for ordinary API
        // routes, but it must not be able to mint per-user Bearer tokens, and
        // rotating it is admin-only.
        const app_source = fs.readFileSync(path.join(__dirname, '..', 'app.js'), 'utf8');
        const jwt_source = fs.readFileSync(path.join(__dirname, '..', 'authentication', 'optional-jwt.js'), 'utf8');

        const route_start = app_source.indexOf("app.post('/api/generateNewAPIKey'");
        assert(route_start !== -1, 'the transitional key rotation route is gone');
        const route_source = app_source.slice(route_start, app_source.indexOf('});', route_start));
        assert(route_source.includes('requireAdmin'),
            'rotating the shared key must be admin-guarded');

        assert(jwt_source.includes('authenticated_with_api_key'),
            'optional-jwt must track global-key sessions');
        assert(/requireJwtForTokenManagement[\s\S]*authenticated_with_api_key/.test(jwt_source),
            'the shared key must be refused for token management');
    });
